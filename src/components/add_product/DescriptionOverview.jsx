      ...
      <Grid container spacing={2.5}>
        <Grid item>
          ...
          <StyledInput
            ...
            value={values.videoUrlForReview}
            onChange={handleChangeVideoReview}
            name="videoUrlForReview"
          />
          {Boolean(errors?.videoUrlForReview) && (
            <Typography component="p" variant="body2" color="error">
              {errors.videoUrlForReview}
            </Typography>
          )}
        </Grid>
        <Grid item {...getRootProps()}>
          ...
          <StyledInput
            startAdornment={<DownloadVideoIcon />}
            placeholder={
              values.pdfFileForReview
                ? values.pdfFileForReview
                : "Вставьте документ в PDF файле"
            }
            title={values.pdfFileForReview}
          />
          {Boolean(error) && (
          ...
          {Boolean(errors?.pdfFileForReview) && (
            <Typography component="p" variant="body2" color="error">
              {errors.pdfFileForReview} 2
            </Typography>
          )}
        </Grid>
        <Grid item xs={12}>
