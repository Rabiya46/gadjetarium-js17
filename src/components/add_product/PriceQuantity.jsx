...
const PriceQuantity = () => {
    ...
    if (Number(value) > 0) {
      dispatch(
        ActionAddProductSlice.editData({
          [editData.isChecked]: Number(value),
        })
      );

      const updateSubProductequests = values.subproducts.map((item, index) => {
        if (index === Number(editData.id)) {
          const newData = {
            ...item,
            [editData.isChecked]: Number(value) ? Number(value) : 1,
          };
          return newData;
        }
        return item;
      ...
      setError(
        `${
          editData.isChecked === "count" ? "Кол-во товара" : "Цена"
  ...
  const nextStepperHandler = () => {
    const emptyFieldsPrice = values.subproducts.map((product, index) => {
      if (!Number(product.price)) {
        return index;
      }
      return null;
    });
    const emptyFieldsCount = values.subproducts.map((product, index) => {
  ...
  return (
    <StyledPriceQuantity component="form" onSubmit={handleSubmit}>
      <Grid container spacing={2.5} className="flex">
        ...
        <Grid item xs={12}>
          <AddProductTable
            tableData={addProductFirstPart.subproducts}
            id={findId}
          />
        </Grid>
