...
export const PRODUCT_INITIALSTATESCHEMA = yup.object().shape({
  ...
  videoUrlForReview: yup
    .string()
    .required("Загрузите видеообзор обязательное поле"),
  descriptionForReview: yup.string().required("Описание обязательное поле"),
  brandId: yup
    .number()
    .positive()
  ...
  subproducts: yup.array().of(
    yup.object().shape({
      ...
      count: yup.number(),
      basicColor: yup.string().required("Color is a required field"),
      images: yup
        .array()
  ...
  pdfFileForReview: yup
    .string()
    .required("Загрузите документ PDF обязательное поле"),
});
    ...
    Cell: ({ row }) => {
      return (
        <div style={generalStyles}>
          <ColorName color={row.original.basicColor} />
        </div>
      );
    },
  },
...
export const ChangeValueTableData = [
  ...
  },
  {
    ...
    Cell: ({ row }) => {
      return <EditCountProduct count={row.original.count} id={row.id} />;
    },
  },
  {
    Header: "Цена",
