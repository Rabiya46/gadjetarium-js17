...
export const addProductSchema = (keys = []) => {
  ...
  const ADDPRODUCT_INITIALSTATESCHEMA = yup.object().shape({
    ...
    createdDate: yup.date().required("Дата выдачи обязательное поле"),

    subproducts: yup.array().of(subProductSchema),
  });
...
export const validationHandler = (categoryId, setKeys, setValues, values) => {
  ...
  if (findedCharacteristicsKeys) {
    ...
    const updatedSubProductRequests = values.subproducts.map((subProduct) => {
    ...
    setValues({
      ...values,
      subproducts: updatedSubProductRequests,
    });
  }
};
