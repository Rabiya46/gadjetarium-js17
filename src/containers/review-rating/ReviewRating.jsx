...
const ReviewRating = () => {
  ...
  const reviews =
    data?.map((item) => ({
      ...
      productReviewResponse: {
        ...
        productVendorCode: item.productArticle,
        image: item.productImages[0] || "",
      },
      userReview: item.text,
      reviewTime: item.createdDate,
      reviewImages: item.images || [],
      productGrade: item.rating,
      userResponse: {
        firstName: item.userFullName,
  ...
  reviews.forEach((r) => {
    const grade = r.productGrade;
    const index = countGradeKey.findIndex((g) => g.key === grade);
    if (index >= 0) {
      countGradeKey[index].value += 1;
      countGrade[index] += 1;
    } else {
      countGradeKey.push({ key: grade, value: 1 });
      countGrade.push(1);
    }
  });

  const count = countGrade.reduce((acc, curr) => acc + curr, 0);
  ...
  const handleTab = (e) => {
    ...
    dispatch(
      getRevewRating(
        e.target.id == 1 ? "all" : e.target.id == 2 ? "unanswered" : "answered"
      )
    );
  };
