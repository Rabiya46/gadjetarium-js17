...
const CatalogProducts = () => {
  ...
  useEffect(() => {
    const getChips = CATALOG_PRODUCTS_FILTERS_KEYS.reduce((acc, key) => {
      const items = Array.isArray(filteredCatalog[key])
        ? filteredCatalog[key]
        : []; // безопасно
      const chipsForKey = items.map((item) => ({
        key,
        title: item,
        id: Math.round(Math.random() * 1000000),
      }));
      return [...acc, ...chipsForKey];
    }, []);
    setChips(getChips);
  }, [filteredCatalog]);
  ...
  useEffect(() => {
    dispatch(
      fetchDataCatalog({
        ...
        screenDiagonals: filteredCatalog.screenDiagonals,
        batteryCapacities: filteredCatalog.batteryCapacities,
        wirelessInterfaces: filteredCatalog.wirelessInterfaces,
        caseShapes: filteredCatalog.caseShapes,
        braceletMaterials: filteredCatalog.braceletMaterials,
      ...
      <Box className="general-box">
        <Box className="filter-box">
          <Typography className="find-products">
            Найдено {data?.products?.size} Товаров
          </Typography>
