...
export const putPhoto = createAsyncThunk(
  "private/putPhoto",
  async ({ id, photo }, { dispatch }) => {
    try {
      const response = await axiosInstance.post(
        `/api/users/${id}/change-photo?photo=${photo}`
      );
      const result = await response.data;
...
export const privateSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(putPhoto.rejected, (state) => {
        state.putPhoto.status = "rejected";
      })
      .addCase(putPhoto.fulfilled, (state, action) => {
        state.putPhoto.status = "fulfilled";
        state.putPhoto.dataa = action.payload;
      })
      .addCase(putPhoto.pending, (state) => {
        state.putPhoto.status = "pending";
      })
